...
int main (int argc, char* argv[]){
    ...
    int mod = nodecount % npes;
    int sum = 0;
    for (i = 0; i < npes; i++) {
        if (i == (npes - 1)) {
            recvcounts[i] = local_node + mod;
        }
        else {
            recvcounts[i] = local_node;
        }
        displacements[i] = sum;
        sum += recvcounts[i];
    }

    local_node = recvcounts[my_rank];
    ...
    do{
        ...
        for ( i = 0; i<local_node; ++i){
            tmp[i] = 0;
            int indx = displacements[my_rank] + i;
        ...
        // Gather everything
        MPI_Allgatherv(tmp, local_node, MPI_DOUBLE, r, recvcounts, displacements, MPI_DOUBLE, MPI_COMM_WORLD);
